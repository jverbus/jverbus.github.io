...
const out = path.resolve('scripts/design-review/demo-polish-2026-09-19/screenshots');
...
(async()=>{
  ...
  async function navigate(key,port=4195){
  ...
  for(const [label,width,dpr] of [['desktop',1280,1],['mobile',390,2]]){
    ...
    for(const scheme of ['light','dark']){
      ...
      for(const key of Object.keys(routes)){
        ...
        if(label==='mobile'&&key==='if'){
          assert(scheme+' mobile maps align side by side',await evaluate(`(()=>{const a=document.querySelector('[data-panel=if]').getBoundingClientRect(),b=document.querySelector('[data-panel=eif]').getBoundingClientRect();return Math.abs(a.top-b.top)<2&&b.left>a.right&&Math.abs(a.width-a.height)<2})()`));
          assert(scheme+' mobile preset buttons form a deliberate grid',await evaluate(`(()=>{const b=Array.from(document.querySelectorAll('[data-if-demo] button')).map(b=>b.getBoundingClientRect());return b.every(r=>r.height>=44)&&Math.abs(b[0].top-b[1].top)<2&&Math.abs(b[2].top-b[3].top)<2&&b[2].top>b[0].bottom})()`));
        }
        if(key==='lux')assert(label+' '+scheme+' LUX controls stay together',await evaluate(`(()=>{const bar=document.querySelector('.lux-demo-controls');return !!bar.querySelector('[data-lux-energy-value]')&&!!bar.querySelector('[data-lux-angle]')&&!!bar.querySelector('button')})()`));
        if(key==='castle'){
          await navigate(key,4196);await shot(label+'-'+scheme+'-castle-baseline',key);
  ...
  await key('Home',36);await key('ArrowRight',39);await sleep(150);
  assert('Styled slider retains native keyboard control',await evaluate(`document.querySelector('[data-lux-energy-value]').textContent==='<0.01 keV'`));
  ...
  for(const k of ['if','lux','orbit']){
    await send('Page.navigate',{url:'http://127.0.0.1:4195'+routes[k]});await poll('document.readyState === "complete"');
